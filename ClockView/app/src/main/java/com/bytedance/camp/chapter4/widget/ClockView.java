...
public class ClockView extends View {
    ...
    // 刻度线的宽度
    private static final float UNIT_LINE_WIDTH = 8;
    private static final int HIGHLIGHT_UNIT_ALPHA = 0xFF;
    private static final int NORMAL_UNIT_ALPHA = 0x80;

    // 指针长度相对表盘半径的比例
    private static final float HOUR_NEEDLE_LENGTH_RATIO = 0.4f;
    private static final float MINUTE_NEEDLE_LENGTH_RATIO = 0.6f;
    private static final float SECOND_NEEDLE_LENGTH_RATIO = 0.8f;

    // 指针的宽度
    private static final float HOUR_NEEDLE_WIDTH = 12;
    private static final float MINUTE_NEEDLE_WIDTH = 8;
    private static final float SECOND_NEEDLE_WIDTH = 4;
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float hourDegree = hour + minute / 60f;
    ...
    private void drawTimeNumbers(Canvas canvas) {
        ...
        for (int i = 0; i < 12; i++) {
            ...
            canvas.drawText(
                    ...
                    (float) (centerY + (radius * 0.8f + textHeight / 2f) * Math.sin(Math.toRadians(hourDegree))),
                    numberPaint
            );
        }
        ...
        canvas.drawText(
                number, centerX, (float) (getHeight() * 0.3), numberPaint
        );
    }
