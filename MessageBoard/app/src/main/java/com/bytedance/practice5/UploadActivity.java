    ...
    private void submit() {
        ...
        if (!UploadParts.isEmpty()) {
            new Thread(new Runnable() {
                @Override
                public void run() {
                    Call<UploadResponse> upResponses = api.submitMessage(STUDENT_ID, "",
                            UploadParts.get("from"), UploadParts.get("to"), UploadParts.get("content"),
                            UploadParts.get("image"), token);
...
//                    strBuf.append(coverImageData);
                    ...
                    if (conn.getResponseCode() == 200) {
                        ...
                        if (RseponseResult.success) {
                            ...
                            Log.d(TAG, "上传失败" + RseponseResult.error);
                            makeUIToast(UploadActivity.this, "上传失败" + RseponseResult.error, Toast.LENGTH_SHORT);
                        }
                        reader.close();
                        is.close();
                    } else {
                    ...
                    makeUIToast(UploadActivity.this, "连接失败" + e.toString(), Toast.LENGTH_SHORT);
                }
            }
        }).start();
    }
